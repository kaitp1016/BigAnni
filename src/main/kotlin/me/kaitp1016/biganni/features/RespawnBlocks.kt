...
object RespawnBlocks: Listener {
    ...
    val respawnableBlocks = hashMapOf(
        ...
        // その他
        Material.GRAVEL to RespawnableBlock(Material.GRAVEL, Material.COBBLESTONE, 500, 2..3, listOf(Material.BONE to -5..3, Material.STRING to -5..3, Material.FLINT to -5..3, Material.FEATHER to -5..3, Material.ARROW to -19..3,)),
    ...
    val placedBlocks = BlockPosInfo<Unit>()
    val respawingBlocks = BlockPosInfo<RespawingBlock>()
    ...
    fun onBreak(event: BlockBreakEvent) {
        ...
        if (respawingBlocks.has(level, pos)) {
            event.isCancelled = true
            return
        }

        val respawnData = respawnableBlocks[block.type] ?: return
        if (placedBlocks.contains(level, pos)) return
        ...
        respawingBlocks[level, pos] = RespawingBlock(level, pos, originalBlock, respawnData.respawnTime)
    ...
    fun onPlace(event: BlockPlaceEvent) {
        ...
        if (respawnableBlocks.contains(block.type)) {
            val level = block.world.toMC()
            placedBlocks[level, BlockPos(block.x, block.y, block.z)] = Unit
        }
    ...
    fun onTick(event: ServerTickStartEvent) {
        respawingBlocks.removeIf { level, pos, block ->
            block.tick--

            if (block.tick < 1) {
                level.setBlockAndUpdate(pos, block.block)
